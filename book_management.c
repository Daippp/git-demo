...
void initbook(Book* b, int id, char* title, char* authors, int year, int copies) {
    ...
    b->authors = (char*)malloc(sizeof(char) * (strlen(authors) - 1));
...
int load_books(FILE* file) {
    ...
    int len = 0;
    bk.list = (Book*)malloc(sizeof(Book));
    Book* p = bk.list;
    ...
    for (i = 0; i < booksum; i++) {
        ...
        p->next = (_Book*)malloc(sizeof(Book));
        p = p->next;
        p->next = 0;
        len++;
        p->id = id;
        ...
        p->authors = (char*)malloc(sizeof(char) * (strlen(author) + 1));
...
int add_book(Book book) {
    ...
    booksum++;
    q->next = (_Book*)malloc(sizeof(Book));
    q = q->next;
    q->next = p;
    q->id = book.id;
    q->year = book.year;
    q->copies = book.copies;
    q->title = (char*)malloc(sizeof(char) * (strlen(book.title) + 1));
    strcpy(q->title, book.title);
    q->authors = (char*)malloc(sizeof(char) * (strlen(book.authors) + 1));
...
BookList find_book_by_title(const char* title) {
    BookList ret;
    ret.list = (Book*)malloc(sizeof(Book));
    Book* r = ret.list;
    ret.list->next = NULL;
    Book* p = bk.list->next;
    while (p) {
        if (strcmp(p->title, title) == 0) {
            r->next = (_Book*)malloc(sizeof(Book));
            r = r->next;
            r->next = NULL;
            r->id = p->id;
            r->year = p->year;
            ...
            r->title = (char*)malloc(sizeof(char) * (strlen(p->title) + 1));
            strcpy(r->title, p->title);
            r->authors = (char*)malloc(sizeof(char) * (strlen(p->authors) + 1));
...
void searchbook() {
    ...
    int option = print_except_number(3, 1, 4, "\nSorry, the option you entered was invalid, please try again.\n");
    char key[1000];
    int t;
    BookList h;
    switch (option) {
